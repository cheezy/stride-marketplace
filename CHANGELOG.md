...
- **`stride` 1.70.0 -> 1.71.0.** Five of the six are the shape the last entry was about — a failure that produced a success. **D287:** D275 closed the KEY channel into the env cache and left the VALUE channel open. A server-supplied `TASK_TITLE` reaches the cache with its newlines intact, so a title carrying a record-shaped line lands *inside* a quoted value; sourcing it is safe, but four line-oriented readers matched that interior line and the one whose output is written back promoted it into a standalone record — after which it is genuine, because the D273 shape check no longer applies to a line that is no longer a continuation. Reproduced end to end; records are now identified by where they *begin*. **D286:** D278 fixed `record_dirty_baseline` on the bash side and left the PowerShell twin listing without `-z`, so it recorded git's octal-escaped display spelling while its own capture recorded the raw one — the two could never match for a non-ASCII path, which made the W1457 pre-existing-edit filter silently do nothing for exactly those files, on Windows only. **D290:** the filter that strips `.stride_auth.md` from a `changed_files` snapshot ran inside a `try` whose `catch` recovered by uploading the **raw, unfiltered** bytes; under `Set-StrictMode -Version Latest` an entry lacking a `path` property throws at the comparison itself, and the body naming the credential file genuinely reached a test listener before the fix. W2100 had closed this fail-open for its one named cause and left its shape. **D288:** every bash record writer piped `grep -v` into a sink that commits whatever reaches its stdin, so a `grep` that declines to read the cache left only the line being added, at exit 0 — measured on a real refusing `grep` rather than a shim, in both its shapes, one of which prints to *stdout* at exit 0 and therefore defeats the emptiness test the defect was filed proposing as the remedy. The load-bearing discovery is that **awk's regex engine is not immune to the same byte**, so swapping one tool for the other would have moved the defect rather than closed it; the filters now match with `index`/`substr` on the ASCII key and propagate their exit status. **D289:** five `Write-EnvCache` callers still committed a whole-file replace against whatever they read whenever they read it, so a claim-side or finalize-side rewrite could discard a record write committed inside its own window; the retry moved into one shared helper and all six go through it.

  **W2107 is the first cross-verified pair in the fleet.** `scripts/check-port-canon.ps1` is an **independent implementation** of the canon drift check, not a transliteration — the bash half tokenises the canon's json with `awk`, this one builds an object graph with `ConvertFrom-Json` — and both halves' self-tests now run inside both suites, never the fleet scan, whose correct result today is exit 1. The cross-check compares the two halves' 100 **case-name sets**, so a case renamed or lost on one side goes red where neither half's own tally could see it, and runs both against one fixture tree. It earned its keep immediately: the review found three divergences **invisible to a fleet-wide diff**, because all three are latent on today's fleet — a case-insensitive hashtable comparer, case-insensitive member access, and three of four vendored directory prunes missing. Three further gaps are recorded in the code rather than closed, each needing one change touching *both* halves.

  **The Windows position is unchanged and the pin's own release says so.** Nothing in that repository has yet *executed* the hook under `powershell.exe` 5.1 (**D237**). `scripts/check-ps1-compat.sh` gates that subset by AST and the new `.ps1` passes it, but a gate that reads is not a run. Stated here for the third entry running, for the same reason: "Windows works now" would be the overstatement those entries were careful to avoid.

This entry does all four steps: the pin bumped, `metadata.version` 1.83.0 -> 1.84.0, the README row and prose section synced, and this entry written. The section cites its own pin as its highest version, keeping the D189 invariant.

**Tagged `v1.85.0` rather than `v1.84.0`.** The tag series is independent of `metadata.version` and `v1.84.0` was spent on the previous entry, for the reason that entry records. The gap between the two records is now three. `marketplace.json` remains the authority on what is pinned, and nothing installs through a tag.
